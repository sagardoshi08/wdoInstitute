...
<style>
   ...
   .form-group select#task-status-drop {
      ...
      <div class="row">
         ...
         <div>
            <div class="row">
               <div class="students-start">
                  <div class="students-text">
                     <h4 class="text-start">Action</h4>
                  </div>
                  <div class="row students-start-col" style="padding-left: 1%;">
                    <form method="POST" action="{{route('updateTaskStatus')}}">
                      @csrf
                     <div class="col-sm-3">
                      <div class="d-flex gap-3">
                        <div class="form-group">
                          <input type="hidden" name="aasign-id" id="aasign-id" value="{{$student->task_id}}">
                          <input type="hidden" name="student-id" id="student-id" value="{{$student->id}}">
                           <label><b>Task Status</b></label>
                           <select class="w-100 required shadow appearance-none border-1 rounded text-gray-700 leading-tight focus:outline-none focus:shadow-outline valid" id="task-status-drop">
                         
                          </span>
                           <option value="">select Task Status</option>
                            @if($assign_role == 'Assinger')
                        ...
                        <div class="col-sm-3 mt-4">
                            <div class="form-group">
                              <button class="rj-btn all-bt taskstatus-update" type="submit">Submit</button>
                          </div>
                        </div>
                        </div>
                     </div>
...
<script type="text/javascript">
   $(document).ready(function() {
    ...
    $(document).on('click', '.taskstatus-update', function() {
         $.ajax({
                type: 'POST',
                url: "{{ route('updateTaskStatus') }}",
                data: {
                  "taskid": $('#aasign-id').val(),
                  'student_id': $('#student-id').val(),
                  'status': $('#task-status-drop').val(),
                  "_token": "{{ csrf_token() }}"
               },
                dataType: 'json',
