...
<div class="inner-table-wrapper page-edit-listing">
<div class="container">
    <a href="{{ route('admin.Page') }}" style="color: black; text-decoration: none;">
    	<h4 class="mt-4">
    		<svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor" class="bi bi-arrow-left-short mr-3" style="color: #333333;" viewBox="0 0 16 16">
 			...
 			<i class="fa fa-hmd-pages mr-2"></i> Back to Pages
 		</h4>
 	</a>
	<div class="edit-profile mt-4">
		<div class="row mt-3 ml-1 mr-2 mb-3">
			<h3><i class="fa fa-hmd-pages update-name"></i><span class="user-name">{{ ucfirst($page->first_name) }} {{ ucfirst($page->last_name) }}</span></h3>
			<div class="row pr-0  success-msg d-none">
        	...
        	<div class="col-xs-12 col-sm-12 col-md-6 col-lg-4">
    			...
    			<div class="form-group">
        			<label>Owner: </label> <span id="update-name">{{ucfirst($page->name)}}</span>
        		</div>
        		<div class="form-group">
        			<label>Previous Owners: </label> <span id="owner">none</span><span class="d-none" id="previous-owner">{{ucfirst($page->name)}}</span>
        		</div>
        		<div class="form-group">
        		...
        		<div class="form-group page-edit-button">
        	...
        	<div class="col-xs-12 col-sm-12 col-md-6 col-lg-4">
        		...
        		@if(!$desc_info && $billing_info)
					<div><h4 class="not-paid">Not Paid</h4></div>
				@else
        		<div class="form-group">
        			<b><label>Status: </label>{{-- {{$billing_info->status ? ucfirst($billing_info->status) : '-'}}--}}</b>
        		</div>
        		<div class="form-group">
        			<label>Name on Card: </label>{{-- {{ucfirst($page->name)}}--}}
        		</div>
        		<div class="form-group">
        		...
        		<div class="form-group" style="position: absolute; top: 268px;">
		        	<button class="btn btn-secondary update-billing" type="button">Update Billing</button>
	        	</div>
	        </div>
	        <div class="col-xs-12 col-sm-12 col-md-6 col-lg-4">
	        		...
	        		<hr class="mt-1">
	        		<div class="form-group">
	        			<b><label>Database size: </label> {{getdatabasesize($page->id)}}</b>
	        		</div>
	        		<div class="form-group">
	    ...
	    <div class="modal-content">
	      	...
	      	<div class="modal-body">
	      		<form method="post" id="page-edit">
	  				@csrf
					<input type="hidden" name="pre_user_id" id="pre-user-id" value="{{$page->customer_id}}">
	  				<input type="hidden" name="id" id="id" value="{{$page->id}}">
	    			<div class="form-group">
	      	    		<label>Owner</label>
		      			<select class="form-control" name="customer_id" id="customer_id">
		      				@foreach($all_owner as $data)
		        				<option value="{{$data->id}}" {{ ($data->id == $page->user_id) ? 'selected' : ''}}>{{$data->name}}</option>
		        			@endforeach
		        		</select>
	      			</div>
		      		<div class="form-group">
		      		...
		      		<div class="form-group" style="display: flex; justify-content: space-between;">
			      		...
			      		<div class="custom-control custom-switch">
						  	<input type="checkbox" name="approval_required" class="custom-control-input" value="{{$page->approval_required }}" id="approval_required">
						  	<label style="position: absolute; right: 85px;" class="off">Off</label>
...
<div class="modal fade admin-modal" id="update-billing-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  	<div class="modal-dialog" style="width: 420px;height: 100vh;display: flex;align-items: center;margin-top: 0;" role="document">
	    <div class="modal-content">
	      	<div class="modal-header" style="margin: 0 24px;">
...
$(document).ready(function(){
	...
	var dataTable = $('.sample_data').DataTable({
		...
		"ajax" : {
	        type:"POST",
			url:'{{route("admin.PageMemberData")}}',
	        data:  function (d) {
	            d.id = {{$page->id}};
	            d.user_id = {{$page->user_id}};
	        },
	        headers: {
    ...
    $(document).on('submit', '#user-member-status', function(event) {
        ...
        $.ajax({
            type:'POST',
            url:"{{ route('admin.pageMemberBlock') }}",
    ...
    $(document).on('submit', '#user-member-unblock', function(event) {
        ...
        $.ajax({
            type:'POST',
            url:"{{ route('admin.pageMemberUnBlock') }}",
            ...
            success:function(data){
                ...
                toastr.success('Memeber Unblocked Successfully');
            }
        });
    });
	...
	$(document).on('click','.update-billing',function() {
     	$('#update-billing-modal').modal('show');
    });
    ...
    $(document).on('submit', '#page-edit', function(event) {
        ...
        $.ajax({
            type:'POST',
            url:"{{ route('admin.PageUpdate') }}",
			data:{
				'id': $('#id').val(),
				'previous_customer_id': $('#pre-user-id').val(),
