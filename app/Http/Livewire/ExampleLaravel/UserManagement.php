...
class UserManagement extends Component
{
    ...
    public function store()
    {
        ...
        $other_details =
        [
            'other_university_name' => $this->other_university_name,
            'other_university_qua_year' => $this->other_university_qua_year,
            'other_university_percentage' => $this->other_university_percentage,
            'other_university_roll_no' => $this->other_university_roll_no
        ...
        $user_address = [
            ...
            'p_landmark' => $this->p_landmark,
            'p_country' => $this->p_country,
            'p_city' => $this->p_city,
            'p_state' => $this->p_state,
            'p_zipcode' => $this->p_zipcode,
            'c_street' => $this->c_street,
            'c_landmark' => $this->c_landmark,
    ...
    public function AssignStore()
    {
        ...
        if(isset($this->assign_user)){
            User::whereIn('uuid',$this->assign_user)->update(['assigned'=> $this->uuid]);
        }else{
            User::where('assigned', $this->uuid)->update(['assigned'=> null]);
        }
    ...
    public function roleStatus(Request $request){
        ...
        if($request->role == 'all_user'){
            $role = 'all_user';
            User::where('role','!=','super_admin')->update(['status' => $status]);
        }elseif($request->role == 'Admin'){
            $role = 'Admin';
            User::where('role','Admin')->update(['status' => $status]);
        }elseif($request->role == 'Manager'){
            $role = 'Manager';
            User::where('role','Manager')->update(['status' => $status]);
        }elseif($request->role == 'Team_Leader'){
            $role = 'Team_Leader';
            User::where('role','Team Leader')->update(['status' => $status]);
        }else{
            $role = 'Employee';
            User::where('role','Employee')->update(['status' => $status]);
        }
        RoleStatus::where('id',1)->update([$role => $status]);
    ...
    public function createuser(Request $request){
        ...
        if ($request->tenth_marksheet) {
            $tenth_marksheet = 'tenth_marksheet_'.$users.'.'.$request->tenth_marksheet->getClientOriginalExtension();
        ...
        if ($request->twelfth_marksheet) {
            $twelfth_marksheet = 'twelfth_marksheet_'.$users.'.'.$request->twelfth_marksheet->getClientOriginalExtension();
        ...
        if ($request->aadhar_card) {
            $aadhar_card = 'aadhar_card_'.$users.'.'.$request->aadhar_card->getClientOriginalExtension();
        ...
        if ($request->alternative_govt_id_card) {
            $alternative_govt_id_card = 'alternative_govt_id_'.$users.'.'.$request->alternative_govt_id_card->getClientOriginalExtension();
        ...
        if ($request->bank_passbook) {
            ...
            // $bank_passbook =  $request->bank_passbook->store('user/bank-passbook');
            // $user_data['bank_passbook'] = $bank_passbook;
        }


        if ($request->graduation_diploma) {
            $graduation_diploma = 'graduation_diploma_'.$users.'.'.$request->graduation_diploma->getClientOriginalExtension();
        ...
        if ($request->post_graduation) {
            ...
            $post_graduation = 'post_graduation_'.$users.'.'.$request->post_graduation->getClientOriginalExtension();
        ...
        if ($request->experience_certificate) {
            ...
            $experience_certificate = 'experience_certificate_'.$users.'.'.$request->experience_certificate->getClientOriginalExtension();
        ...
        if ($request->salary_slip) {
            ...
            $salary_slip = 'salary_slip_'.$users.'.'.$request->salary_slip->getClientOriginalExtension();
        ...
        if ($request->crope_img) {
            ...
            $image_name= 'profile_image_'.$users.'.png';
    ...
    public function applyJobCreate(Request $request){
        $uuid = (string) Str::uuid();
        ...
        return back()->with(session()->flash(
    ...
    {
        User_data::where('user_id',$request->id)->delete();
        User_address::where('user_id',$request->id)->delete();
        AssignTask::where('employee_id',$request->id)->delete();
