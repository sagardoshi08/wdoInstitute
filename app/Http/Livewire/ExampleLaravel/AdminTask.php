...
class AdminTask extends Component
{
    ...
    public function render()
    {
            ...
            $students = Student::whereIn('id',$assign)->where('student_name', 'like', '%' . $this->search_name . '%')->get();
            $admin = User::where('role','Admin')->get();
            $manager = User::where('role','Manager')->get();
            $team_leader = User::where('role','Team Leader')->get();
            $employee = User::where('role','Employee')->get();

        return view(
            'livewire.example-laravel.assigntask.admin-task',
            [
                'users' => $user,
                'user_data' =>$user_data,
                'students' => $students,
    ...
    {
        $assign = AssignTask::select('student_id')->get()->toArray();
        $students = Student::whereNotIn('id',$assign)->get();

        return view('livewire.example-laravel.assigntask.student-details',compact('students','id'));
    }
    public function assigntask(Request $request){

        $student_id = explode(',',$request->student_id);
        //echo '<pre>'; print_r($student_id[0]); die();
        ...
        foreach($student_id as $key=>$data){
            AssignTask::create([
                ...
                'contacts_permission' => $request->contacts_permission,
                'aadhar_permission' => $request->aadhar_permission,
                'application_permission' => $request->application_permission,
                'bank_permission' => $request->bank_permission
            ]);
        }
        $user_type = User::select('role')->where('id',$request->employee_id)->first();
        if($user_type->role == 'Admin'){
            $url = 'admin-task';
        }elseif($user_type->role == 'Manager'){
            $url = 'manager-task';
        }elseif($user_type->role == 'Team Leader'){
            $url = 'teamleader-task';
        }else{
            $url = 'employee-task';
        }
        return redirect()->route('assign-task')->with(session()->flash(
    ...
    public function getassignstudentid(Request $request){
        ...
        if ($request->input("search")["value"] && $request->input("value") == '') {
            $students = $students->where('student_name', 'like', '%' . $request->input("search")["value"] . '%');
        }
        $filter_count = $students->get()->count();
        if ($request->input('length') != -1) {
            $students = $students->offset($request->input('start'))->limit($request->input('length'));
        }
        ...
        foreach ($students as $key=>$row) {
            $sub_array = array();
             $sub_array['input'] = $row->id;
            $sub_array['sr_no'] = ++$key;
            $sub_array['student_name'] = $row->student_name;
            $sub_array['application_number'] = $row->application_number;
            $sub_array['dob'] = $row->dob;
            $sub_array['bank_name'] = $row->bank_name;
            $sub_array['account_number'] = $row->account_number;
            $sub_array['year'] = $row->year;
            $sub_array['college_name'] = $row->college_name;
            $sub_array['course_details'] = $row->course_details;
            $sub_array['status'] = '1';
            $sub_array['action'] = $row->id;
            $data[] = $sub_array;
        }

        $output = array(
            "draw" => intval($request->input('draw')),
            "recordsTotal" => Student::whereIn('id',$id)->count(),
    ...
    public function removetask(Request $request){
        ...
        foreach($array as $key=>$data){
            AssignTask::where('student_id',$data)->delete();
        }

        echo json_encode(true);
    }

    public function getreassigneuser(Request $request){
    ...
    public function change_permission(Request $request){
        ...
        $employee = $request->employee_id;
        foreach($array as $key=>$data){
            AssignTask::where('student_id',$data)->where('employee_id',$employee)->update([
                'contacts_permission' => $request->contacts_permission,
                'aadhar_permission' => $request->aadhar_permission,
                'application_permission' => $request->application_permission,
                'bank_permission' => $request->bank_permission
    ...
    public function update_reassign_task(Request $request){
        //echo '<pre>'; print_r($request->all());
        $array = explode(',',$request->student_id);
        $employee = $request->employee_id;
        foreach($array as $key=>$data){
